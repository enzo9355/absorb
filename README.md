...
| `stock_papi/web/app_factory.py` | Flask app factory；啟動時不刷新雲端憑證 |
| `stock_papi/application.py` | 尚待逐步拆分的 legacy routes、LINE 與分析協調層 |
| `stock_papi/settings.py` | 輕量環境設定與非模型限制值 |
| `stock_papi/shared/` | 無 Flask／網路副作用的格式化與驗證 helper |
| `stock_papi/integrations/line/flex.py` | 純 LINE Flex presentation builders |
...
新增 route 時應透過 Web registration 邊界並保留明確 endpoint name；新增業務協調先放入 `stock_papi/services/`，不要再擴充 `stock_papi/application.py`。`stock_papi/**/__init__.py` 必須保持輕量，pandas、numpy、sklearn、LightGBM、PDF 與 Gemini 套件只能在實際執行路徑延遲載入。根目錄 `app.py` 的 module alias 是過渡相容層，用來維持既有 `import app as stock_app` 與 `patch.object` 行為。
