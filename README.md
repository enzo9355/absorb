...
- 市場 universe 全部完成且沒有待重試股票後，才會在 `publish/quant/v1` 建立內容雜湊物件、immutable manifest 與原子 `latest-TW.json`／`latest-US.json`。缺檔、gzip 損壞、schema 不符或 SHA 驗證失敗時保留上一版。
...
- 美股目前支援標準英文字母代碼直接查詢；本地建置器會依 SEC universe 逐夜增量掃描，但 Cloud Run 尚未讀取本地發布 manifest，因此線上產業排名仍沿用既有快照。美股外資、融資融券欄位會顯示資料不足。
