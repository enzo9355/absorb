...
- Web 完整分析頁
  - `/`、`/dashboard`：股票搜尋、市場摘要、今日焦點、產業熱力圖、產業預測、精選標的、新手投資小辭典
  - 搜尋支援台股代碼／名稱與美股代碼，例如 `2330`、`台積電`、`AAPL`
  - `/stock/<code>`：分區導覽、Papi 判讀、產業同儕、互動式 K 線圖、五日預測軌跡、技術指標、投資金額快捷試算、歷史回測、外資買賣超、情緒量化拆解、新聞篩選與風險提示
  - `/market`：台股大盤分析
  - `/market-map`：產業強弱、上市／上櫃 MOPS、ETF 前十大持倉與台美日供應鏈角色地圖
...
正式 Cloud Run 使用 GCP Secret Manager 注入敏感值：

| 環境變數 | Secret |
| --- | --- |
| `FINMIND_USER` | `stock-papi-finmind-user` |
| `FINMIND_PASSWORD` | `stock-papi-finmind-password` |
| `GEMINI_API_KEY` | `stock-papi-gemini-api-key` |
| `LINE_CHANNEL_ACCESS_TOKEN` | `stock-papi-line-channel-access-token` |
| `LINE_CHANNEL_SECRET` | `stock-papi-line-channel-secret` |
| `ALERT_TASK_TOKEN` | `stock-papi-alert-task-token` |

`GCP_PROJECT_ID` 與 `QUANT_SNAPSHOT_BUCKET` 不含憑證，維持一般環境變數。正式服務只參考公開產品功能概念，不呼叫、抓取或複製 aistockmap.com 的 API、資料與視覺資產。
...
- 排程會先建立 `latest-insights.json`：MOPS 使用 TWSE 上市公司每日重大訊息與 TPEx 上櫃公司重大訊息；ETF 持倉由既有 yfinance 在本地取得，單一來源失敗時保留前版；供應鏈採 Stock Papi 自有角色分類，不宣稱未驗證的客戶或供應契約。
...
- 市場洞察沿用相同內容定址物件與 SHA-256 驗證，先上傳 `objects/<sha>.json.gz`，最後才更新 `latest-insights.json`。Cloud Run 最多接受 7 日內、5MB 以下且 schema 完整的快照；失敗時只顯示既有產業訊號與靜態供應鏈，不在雲端補抓資料。
