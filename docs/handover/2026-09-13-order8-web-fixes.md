...
`scripts/check_ci.sh`，三個 head（`c0ca906`／`a5f6995`／`fb04322`）都綠 ——
但那是 CI 跑的，不是我跑的。

**這份交接已經被執行過**：審核端（Codex）在本機跑完整套件並複審後，推了兩個
修正提交（`c4c40fd`、`fb04322`），補掉第 5 節點名的其中一項真缺陷與兩項
規範落差。第 9 節記錄那次審核的結果，以及我的離線驗證為什麼沒抓到。
...
CI 測試範圍），`c0ca906`／`a5f6995`／`fb04322` 三個 head 的兩個版本都 success。
...
## 9. 審核結果（`c4c40fd`、`fb04322`）

審核端在本機跑完整套件（1516 tests 通過、skipped 1）並複審後推了兩個修正。
我逐一重現與驗證過，記在這裡，因為其中一項是我留下的真缺陷：

**9-1 缺值時把 fallback 刻度印成資料（真缺陷，我的）**

長條的 `progress` 需要一個不為 0 的 `max`，所以我在缺值時讓刻度退回 1。
問題是說明文案也直接引用同一個變數，於是四個期間報酬全部缺值時，畫面會印出
「長條共用同一個刻度（最長的一根＝**1.00%**）」、極值那段印出「最長的一根＝**1 檔**」——
那個 1 是計算用的預設值，不是任何一筆已驗證資料。這違反 §19「缺值不是數值」
與 §20「不以推估值補位」，而且正是這個 repo 最在意的那一類錯誤。

已用缺值與全零兩種快照重現過舊版（`a5f6995`）確實會印出那兩個數字；
修正後改成 `return_peak`／`extreme_peak`（真實最大值，可以是 none 或 0）與
`return_scale`／`extreme_scale`（給 `progress` 用、永不為 0）兩個變數，文案只引用前者，
並把「沒有已驗證資料」與「資料是 0」分成兩句不同的話。

**9-2 散戶入口出現「回測」字眼（規範落差）**

§18 明文寫「散戶入口不得出現『回測』『勝率』等績效字眼，連指路文案也不行」。
我從個股頁沿用了「未經回測校準」這句話到市場實況，dashboard 上那句是更早就有的。
兩處都改成「機率值尚未校準，不能當成實際上漲機會解讀」—— 未校準這件事仍然講，
只是不再用績效術語講。我自己那個 `assertNotIn("回測")` 的斷言只覆蓋 promoted 模式，
research 模式那條分支沒被掃到，所以沒紅。

**9-3 長條列共用欄寬（強化，非當下可見的缺陷）**

原本每個 `.viz-row` 各自宣告 `grid-template-columns`，同一份清單的各列理論上可以
算出不同欄寬，長條起點就不對齊、長度也不可比。改為 `.viz-rows` 定義欄、各列用
`subgrid`，並保留一行非 subgrid 的宣告給不支援的瀏覽器。

誠實地說：我用目前的標籤與數值量過舊版與新版的實際幾何（一般、缺一個值、
全缺三種快照，桌機寬度），左緣與寬度完全相同 —— 因為 `minmax(96px,auto)` 與
`minmax(76px,auto)` 的下限在現有文案下都吃得住。所以這一項是把「碰巧對齊」
換成「保證對齊」，不是修一個看得到的歪掉。新增的測試把這個保證釘住。

**我的離線驗證為什麼沒抓到 9-1**

我只 render 了 happy-path 的那一份 fixture（所有欄位都有值），沒有 render
缺值與全零的變體。掃描式不變量檢查的是模板原始碼的寫法（例如「單位不得脫離數值」），
抓不到「這個變數在缺值時的值是多少、會不會被當成資料印出來」——
那只有真的用缺值資料 render 一次才看得到。下次做這類視覺化，
缺值／全零的 render 要跟 happy path 一起做。
