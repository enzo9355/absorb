...
$Year = ([datetime]::ParseExact([string]$Manifest.market_as_of, 'yyyy-MM-dd', $null)).Year
...
& $PythonExe -m reporting.backfill --root $DataRoot --market TW --report-type post_close --source-market-date $Manifest.market_as_of --source-manifest "quant/v1/$ManifestRelative" --source-manifest-sha256 $Latest.manifest_sha256 --model-version $Versions[0] --calendar-artifact $CalendarPath --apply
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
