...
**同一份已驗證的估計，在每個畫它的入口都要畫。** 今日市場與市場實況是同一個指數、同一份產物；一邊有線、一邊沒有，讀者會以為預測被撤下或根本不存在。缺席只能有一個原因：那一天沒有通過驗證的產物 —— 那時兩邊都不畫，而且都要說「尚未發布」。

**模型的歷史績效不出現在散戶入口。** 勝率、最大回撤、Brier Score、特徵重要度只出現在每日報告的研究版（「模型驗證與回測」章節），不出現在個股頁、今日、市場、產業等散戶入口 —— 那些頁面只呈現已驗證的實際市場資料。

這是產品決定，不是因為資料不存在：欄位在資料層一直都在，是 `model_evidence` 的 `performance_endorsement_allowed` 閘門在 baseline 通過相容性驗證前刻意不放行。**即使閘門日後通過，散戶入口仍然不顯示。**

散戶入口不得出現「回測」「勝率」等績效字眼，**連指路文案也不行** —— 那會把術語帶回散戶入口。但「要深挖去報告找」必須是一條真的走得到的路：個股頁的模型估計區塊附一句話，指向每日報告的研究版與該章節名稱。

**預測區間只能呈現實際量到的誤差分布。** 五日預測的誤差區間取自模型過去樣本外誤差的中間 80%，套在點預測上。它是對誤差分布的描述，**不是信賴區間** —— 模型沒有輸出校準過的機率分布，寫成信賴區間等於宣稱一個沒有被驗證過的東西。畫面文案必須同時說出三件事：它是什麼（過去 N 個樣本外測試的誤差中間 80%）、它不是什麼（不是信賴區間）、它不保證什麼（不保證這次落在裡面）。

區間視覺用平色半透明（同預測線色系、18% 不透明度），不加漸層、不加光暈。帶必須畫在 K 線之前 —— 圖表依建立順序繪製，遮罩層若在 K 線之後建立會蓋掉最後一根 K 棒。樣本數不足以估計分位數時不給區間：一個估不準的區間比沒有區間更糟。
...
**密度目標**：主版面的「今天怎麼了」與「為什麼會這樣」的第一張圖必須落在第一個 800px 畫面內（已驗證資料的情境）。事實列在窄寬度維持兩欄，不逐項堆疊成一列一項 —— 那只會把下一段推出畫面。「最新更新」是報告與觀察的入口清單，不是今天的市場本身，因此排在證據之後；入口不刪除，只是不佔頭版。
...
**同一個類名不得同時是「一行摘要」與「版面容器」。** 兩者都宣告 display 時，後出現的規則會套在另一個身上：`.report-track` 既是研究版報告的分頁容器、又被拿來當索引卡上的一行 flex 摘要，結果整份報告的章節被擠成一個字一行的窄欄。新增元件前先在 CSS 裡搜過類名，版面容器與內容元件分開命名。
