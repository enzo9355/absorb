...
- 定位：受邀同學試用的條件式交易建議（us_daily_breakout_v1，日線規則試用版，尚未驗證獲利能力）；手動交易，不自動下單、不接券商、不收集券商密碼。
- 三種來源標籤並列：當事人觀點、揭露事實、ABSORB 規則建議；外部樂觀與數據不符時並列矛盾，不投票升級買進。
- 方向分數不得變成百分比勝率；四種動態（官方交易揭露／當事人自述／機構季底／公開觀點）一眼可辨。
- 沿用品牌與元件，不做整站視覺重設；手機 390px 單欄，人物／行為／交易基準日／揭露日不可隱藏。
