...
- 單人驗證步驟（待使用者在候選版按一次 LINE 登入後執行；全唯讀＋一次存取皆經本人帳號）：
  ...
  4. 回饋／匯出僅本人（匯出不含憑證）
- 生產 wiring 缺口已補（見下）；候選版已重建（含 allowlist 修復；部署源為乾淨
  worktree；主工作樹他人未提交的變更未納入）。當前候選見 CANDIDATE-READBACK 最新一節。
...
- 候選版：新 revision 零流量（正式 100% 未動；read-back 見 CANDIDATE-READBACK.md）
